...
def main(filename, model, tokenizer, max_new_tokens, max_new_seq_len, use_aug, use_labels):
    ...
    assert 'sharegpt' in filename.lower()
    ...
    for i in tqdm(range(len(train_dataset))[:1]): 
        d = train_dataset[i]
        #if len(d["conversations"]) > 2:
        #    continue
    ...
    new_file_name = "cleaned_" + f"sharegpt_jacobi_max_new_tokens{max_new_tokens}_aug{use_aug}_labels_{use_labels}_max_seq_len_{max_new_seq_len}.json"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--filename", type=str,
                        default="sharegpt_20230521_2k_clean_lang_split_identity_gpt4.json")
    ...
    # for sharegpt dataset generation
    # CUDA_VISIBLE_DEVICES=7 python generate_trajectory_sharegpt.py --model path_to_target_model --filename ./raw_data/sharegpt_20230521_2k_clean_lang_split_identity_gpt4.json --use_aug --use_labels --max_new_tokens 16 --max_new_seq_len 512
