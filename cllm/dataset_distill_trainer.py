...
class DistillTrainer(Trainer):
    def __init__(self, teacher_model, *args, **kwargs):
        ...
        self.ar_sample_from_teacher = False
    ...
    def consistency_training_step(self, model, inputs):
        ...
        if self.ar_sample_from_teacher:
        ...
        else: # use labels to avoid pattern collapse
            labels = inputs['labels_ids']
            # TODO: check if it's right when batch size > 1
            labels = torch.tensor(labels).to(jacobian_trajectory[0].device)
            labels = torch.unsqueeze(labels, dim=0)
            attention_mask = torch.full_like(labels, 1).to(labels.device)
